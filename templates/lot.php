<section class="lot-item container">
    ...
    <div class="lot-item__content">
        ...
        <div class="lot-item__right <?=(!isset($_SESSION['user'])) ? "visually-hidden" : "";?>">
            ...
            <form class="lot-item__form <?=(strtotime($lot['dt_end']) < time() || $lot['user_id'] === intval($user_id) || intval($user_id) === $max_bet['user_id']) ? "visually-hidden" : "";?> "action="lot.php?id=<?=$lot['id']; ?>" method="post" autocomplete="off">
                <p class="lot-item__form-item form__item <?=isset($error) ? "form__item--invalid" : "";  ?>">
                <label for="cost">Ваша ставка</label>
                <input id="cost" type="text" name="cost" placeholder="<?=esc($lot['min_bet']);?>" value="<?=esc($my_bet['cost'] ?? ""); ?>">
                <?php if (isset($error)) : ?>
